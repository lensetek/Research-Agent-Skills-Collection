...
* **🇮🇩 Optimisasi Arsitektur Hybrid Python + LLM (Model Lemah) - NEW**: Menambahkan infrastruktur terprogram berbasis Python helper scripts (`json_repair.py`, `progress_tracker.py`, `parse_sections.py`, `build_sota_matrix.py`, `run_stat_analysis.py`) pada skill penulisan literature review, analisis statistika, dan orkestrasi. Menjamin jalannya riset secara terstruktur, cepat, murah, dan 100% bebas halusinasi angka/DOI meskipun dijalankan oleh model LLM kecil (seperti 7B/8B lokal).
* **🇬🇧 Hybrid Python + LLM Optimization Architecture (Weak Models) - NEW**: Introduced Python-based CLI helpers (`json_repair.py`, `progress_tracker.py`, `parse_sections.py`, `build_sota_matrix.py`, `run_stat_analysis.py`) to the orchestrator, literature reviews, and data science skills. Guarantees highly structured, fast, cost-efficient, and 100% hallucination-free research outputs even when run on small/local LLMs (like 7B/8B models).
