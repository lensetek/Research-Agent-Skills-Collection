...
2. **Dynamic Content & Bot Protection Fallback (Robustness Second)**: Jika data dimuat secara dinamis via JavaScript, situs dilindungi bot protection (seperti Cloudflare dasar), atau memerlukan interaksi UI yang kompleks:
   ...
   - **Zero-Failure Auto-Provisioning Rule**: Jika `chrome-devtools` belum aktif, agen DILARANG menjawab "not installed". Agen WAJIB otomatis mengeksekusi pemicuan langsung *on-demand*: `npx -y chrome-devtools-mcp@latest` atau `py bin/mcp_auto_updater.py`.
