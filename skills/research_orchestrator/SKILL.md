...
13. Jalankan `citation-and-reference-validator` untuk memverifikasi keakuratan DOI dan memastikan klaim naskah didukung secara faktual. **WAJIB jalankan skrip `validate_references.py` dan `fetch_evidence_snippet.py` dari folder `scripts/` skill terkait** untuk memfilter referensi palsu dan memvalidasi *Evidence Snippets* verbatim sebelum diserahkan ke fase sintesis.
